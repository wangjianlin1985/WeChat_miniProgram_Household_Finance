...
Page({
  ...
  // 加载收入类型列表
  listIncomeType: function () {
    var self = this
    var url = config.basePath + "api/incomeType/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          incomeTypes: self.data.incomeTypes.concat(res.data.list),
  ...
  //删除收入类型记录
  removeIncomeType: function (e) {
    var self = this
    var incomeTypeId = e.currentTarget.dataset.incometypeid
    wx.showModal({
      title: '提示',
      content: '确定要删除incomeTypeId=' + incomeTypeId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/incomeType/delete/" + incomeTypeId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除收入类型后客户端同步删除数据
            var incomeTypes = self.data.incomeTypes;
            for (var i = 0; i < incomeTypes.length; i++) {
              if (incomeTypes[i].incomeTypeId == incomeTypeId) {
                incomeTypes.splice(i, 1)
                break
              }
            }
            self.setData({ incomeTypes: incomeTypes })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
