...
Page({
  ...
  // 加载支出类型列表
  listExpendType: function () {
    var self = this
    var url = config.basePath + "api/expendType/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          expendTypes: self.data.expendTypes.concat(res.data.list),
  ...
  //删除支出类型记录
  removeExpendType: function (e) {
    var self = this
    var expendTypeId = e.currentTarget.dataset.expendtypeid
    wx.showModal({
      title: '提示',
      content: '确定要删除expendTypeId=' + expendTypeId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/expendType/delete/" + expendTypeId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除支出类型后客户端同步删除数据
            var expendTypes = self.data.expendTypes;
            for (var i = 0; i < expendTypes.length; i++) {
              if (expendTypes[i].expendTypeId == expendTypeId) {
                expendTypes.splice(i, 1)
                break
              }
            }
            self.setData({ expendTypes: expendTypes })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
