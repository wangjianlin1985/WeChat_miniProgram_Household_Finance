	...
	incomeType_manage_tool = {
				...
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						for (var i = 0; i < rows.length; i ++) {
							incomeTypeIds.push(rows[i].incomeTypeId);
						}
						$.ajax({
							type : "POST",
							url : "IncomeType/deletes",
							data : {
								incomeTypeIds : incomeTypeIds.join(","),
							},
							beforeSend : function () {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "IncomeType/" + rows[0].incomeTypeId +  "/update",
					type : "get",
					data : {
						//incomeTypeId : rows[0].incomeTypeId,
					},
					beforeSend : function () {
						$.messager.progress({
							text : "正在获取中...",
