		...
		<div class="col-md-12 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" class="active"><a href="#incomeTypeListPanel" aria-controls="incomeTypeListPanel" role="tab" data-toggle="tab">收入类型列表</a></li>
			    	<li role="presentation" ><a href="<%=basePath %>IncomeType/incomeType_frontAdd.jsp" style="display:none;">添加收入类型</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>收入类别id</td><td>收入类别名称</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
				    	            		...
				    	            		for(int i=0;i<incomeTypeList.size();i++) {
					    	            		...
					    	            		IncomeType incomeType = incomeTypeList.get(i); //获取到收入类型对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											<td><%=incomeType.getIncomeTypeId() %></td>
 											<td><%=incomeType.getIncomeTypeName() %></td>
 											<td>
 												<a href="<%=basePath  %>IncomeType/<%=incomeType.getIncomeTypeId() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="incomeTypeEdit('<%=incomeType.getIncomeTypeId() %>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="incomeTypeDelete('<%=incomeType.getIncomeTypeId() %>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;收入类型信息编辑</h4>
      </div>
...
/*删除收入类型信息*/
function incomeTypeDelete(incomeTypeId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url : basePath + "IncomeType/deletes",
			data : {
				incomeTypeIds : incomeTypeId,
			},
