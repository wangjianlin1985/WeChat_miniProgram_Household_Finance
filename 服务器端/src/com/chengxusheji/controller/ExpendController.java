...
public class ExpendController extends BaseController {
	...
	public String frontlist(@ModelAttribute("expendTypeObj") ExpendType expendTypeObj,String expendPurpose,@ModelAttribute("payWayObj") PayWay payWayObj,String payAccount,String expendDate,@ModelAttribute("userInfoObj") UserInfo userInfoObj,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (expendPurpose == null) expendPurpose = "";
		if (payAccount == null) payAccount = "";
		if (expendDate == null) expendDate = "";
		List<Expend> expendList = expendService.queryExpend(expendTypeObj, expendPurpose, payWayObj, payAccount, expendDate, userInfoObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    expendService.queryTotalPageAndRecordNumber(expendTypeObj, expendPurpose, payWayObj, payAccount, expendDate, userInfoObj);
	    /*获取到总的页码数目*/
	    int totalPage = expendService.getTotalPage();
	    ...
	    request.setAttribute("expendTypeObj", expendTypeObj);
	...
	public String delete(@PathVariable Integer expendId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  expendService.deleteExpend(expendId);
	            request.setAttribute("message", "支出信息删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "支出信息删除失败!");
				return "error";
	...
	/*按照查询条件导出支出信息信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(@ModelAttribute("expendTypeObj") ExpendType expendTypeObj,String expendPurpose,@ModelAttribute("payWayObj") PayWay payWayObj,String payAccount,String expendDate,@ModelAttribute("userInfoObj") UserInfo userInfoObj, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(expendPurpose == null) expendPurpose = "";
        if(payAccount == null) payAccount = "";
        if(expendDate == null) expendDate = "";
        List<Expend> expendList = expendService.queryExpend(expendTypeObj,expendPurpose,payWayObj,payAccount,expendDate,userInfoObj);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Expend信息记录"; 
        String[] headers = { "支出id","支出类型","支出用途","支付方式","支付账号","支付金额","支付日期","用户","备忘信息"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<expendList.size();i++) {
        	Expend expend = expendList.get(i); 
        	dataset.add(new String[]{expend.getExpendId() + "",expend.getExpendTypeObj().getExpendTypeName(),expend.getExpendPurpose(),expend.getPayWayObj().getPayWayName(),expend.getPayAccount(),expend.getExpendMoney() + "",expend.getExpendDate(),expend.getUserInfoObj().getName(),expend.getMemo()});
        }
        /*
