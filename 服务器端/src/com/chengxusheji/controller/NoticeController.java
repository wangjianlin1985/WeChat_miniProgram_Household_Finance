...
public class NoticeController extends BaseController {
	...
	public void list(String title,String addDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (title == null) title = "";
		if (addDate == null) addDate = "";
		if(rows != 0)noticeService.setRows(rows);
		List<Notice> noticeList = noticeService.queryNotice(title, addDate, page);
	    /*计算总的页数和总的记录数*/
	    noticeService.queryTotalPageAndRecordNumber(title, addDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String title,String addDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (title == null) title = "";
		if (addDate == null) addDate = "";
		List<Notice> noticeList = noticeService.queryNotice(title, addDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    noticeService.queryTotalPageAndRecordNumber(title, addDate);
	    /*获取到总的页码数目*/
	    int totalPage = noticeService.getTotalPage();
	...
	public void OutToExcel(String title,String addDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(title == null) title = "";
        if(addDate == null) addDate = "";
        List<Notice> noticeList = noticeService.queryNotice(title,addDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Notice信息记录"; 
        String[] headers = { "记录id","标题","发布日期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<noticeList.size();i++) {
        	Notice notice = noticeList.get(i); 
        	dataset.add(new String[]{notice.getNoticeId() + "",notice.getTitle(),notice.getAddDate()});
        }
        /*
