...
public class UserInfoController extends BaseController {
	...
	public String delete(@PathVariable String user_name,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  userInfoService.deleteUserInfo(user_name);
	            request.setAttribute("message", "家庭成员删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "家庭成员删除失败!");
				return "error";
	...
	public void OutToExcel(String user_name,String name,String birthday, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "亲友账号","姓名","性别","家庭关系","职业","出生日期","用户照片"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getName(),userInfo.getSex(),userInfo.getHomeName(),userInfo.getWorkName(),userInfo.getBirthday(),userInfo.getUserPhoto()});
        }
        /*
